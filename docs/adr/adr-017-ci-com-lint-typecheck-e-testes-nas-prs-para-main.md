...
- Decisão: um workflow `.github/workflows/ci.yml` com um job `ci` em `ubuntu-latest`, disparado por `pull_request` com base `main` (nunca `pull_request_target`), com a permissão `contents: read` e sem segredo algum — PR de fork roda igual. Os passos, nesta ordem, são `actions/checkout@v4`, `pnpm/action-setup@v4` (sem `version:` fixo: lê o `packageManager` da raiz, hoje `pnpm@12.4.1`), `actions/setup-node@v4` com `node-version: 24` e `cache: pnpm`, `pnpm install --frozen-lockfile`, e então um passo por gate: `pnpm lint`, `pnpm typecheck` e `pnpm test`.
